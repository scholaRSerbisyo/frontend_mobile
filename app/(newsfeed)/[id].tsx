...
export default function ContentScreen() {
  ...
  const fetchEventComments = useCallback(async () => {
    try {
      ...
      const uniqueComments = result.comments.map(comment => ({
        ...comment,
        comment_id: comment.comment_id || Date.now()
      }))
      setComments(prevComments => [...prevComments, ...uniqueComments])
      setTotalPages(result.lastPage)
      uniqueComments.forEach(comment => fetchCommentReplies(comment.comment_id))
    } catch (error) {
  ...
  const handleCommentSubmit = useCallback(async () => {
    ...
    try {
      ...
      const uniqueNewComment = {
        ...newComment,
        comment_id: newComment.comment_id || Date.now()
      }
      setComments(prevComments => [uniqueNewComment, ...prevComments])
      setCommentText('')
  ...
  const handleReplySubmit = async () => {
    if (replyText.trim() === '' || selectedCommentId === null) {
      Alert.alert('Error', 'Please enter a reply and select a comment to reply to.');
      return;
    }

    try {
      const newReply = await postReply(selectedCommentId, replyText.trim());
      const uniqueNewReply = {
        ...newReply,
        reply_id: newReply.reply_id || Date.now()
      };
      setReplies(prevReplies => ({
        ...prevReplies,
        [selectedCommentId]: [...(prevReplies[selectedCommentId] || []), uniqueNewReply]
      }));
      setReplyText('');
  ...
  const renderItem = ({ item: comment }: { item: Comment }) => (
    <View style={styles.commentThread}>
      ...
      {(userRole === 1 || userRole === 2) && replies[comment.comment_id]?.map((reply) => (
        <View key={`reply-${comment.comment_id}-${reply.reply_id || Date.now()}`} style={[styles.comment, styles.replyComment]}>
          ...
          <View style={styles.contentCard}>
            <View style={styles.content}>
              ...
              <View style={styles.detailsContainer}>
                ...
                <Feather name="calendar" size={20} color="#000000" style={styles.icon} />
                  <Text style={styles.detailText}>{formatDate(event.date)}</Text>
                </View>

                <View style={styles.detailRow}>
                <Feather name="clock" size={20} color="#000000" style={styles.icon} />
                  <Text style={styles.detailText}>{`${formatTime(event.time_from)} to ${formatTime(event.time_to)}`}</Text>
                </View>

                <View style={styles.detailRow}>
                <Feather name="map-pin" size={20} color="#000000" style={styles.icon} />
                  <Text style={styles.detailText}>{event.location}</Text>
                </View>

                <View style={styles.detailRowLast}>
                  <Text style={styles.detailLabel}>Type of Event:</Text>
                  <Text style={styles.detailText}>{event.event_type.name} based</Text>
                </View>
